...
function useLocationTracker() {
  ...
  const handleLocationUpdate = useCallback((data) => {
    console.log('Nueva ubicación recibida vía SSE:', data);
  ...
  useEffect(() => {
    if (isConnected) {
      console.log('📡 Configurando listeners de SSE...');
  ...
  useEffect(() => {
    if (sseError && !error) {
      setError(`Error de SSE: ${sseError}`);
    }
  }, [sseError, error]);
...
const ConnectionStatus = ({ connectionStatus, isConnected }) => {
  const getStatusColor = () => {
    switch (connectionStatus) {
      ...
      case 'error': case 'disconnected': return 'text-red-400';
  ...
  const getStatusIcon = () => {
    switch (connectionStatus) {
      ...
      case 'connecting':
      case 'reconnecting':
        return <div className="w-3 h-3 bg-yellow-400 rounded-full animate-bounce"></div>;
      case 'error':
      case 'disconnected':
  ...
  const getStatusText = () => {
    switch (connectionStatus) {
      case 'connected': return 'Conectado via SSE';
  ...
  <div className='glassmorphism-strong rounded-4xl max-w-[100%] p-8'>
    ...
    <div className='flex flex-row justify-between gap-4 glassmorphism group hover:scale-105 hover:shadow-[0px_3px_15px_0px_rgba(142,81,255,0.6)] rounded-xl mb-3 pl-2 pr-6 py-2'>
      <div className='flex flex-row gap-2 group justify-left transition-all duration-300 group-hover:scale-105'>
        ...
        <h3 className='text-l text-white rounded-xl inline-block'>Fecha:</h3>
      </div>
...
function App() {
  const { latestLocation, loading, error, refresh, connectionStatus, isConnected } = useLocationTracker();
        ...
        {loading ? (
          ...
          <div className="glassmorphism-strong min-w-[90%] mx-auto rounded-4xl p-8 text-center">
            ...
            <p className="text-white/50 text-sm mb-4">
              {isConnected ? 'Conectado via SSE' : 'Conectando...'}
            </p>
            <button onClick={refresh} className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg transition-colors">
              Refrescar
            </button>
          </div>
        )}
